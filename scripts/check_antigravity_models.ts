...
import { loadToken } from "../src/lib/antigravity/auth";
...
if (!TOKEN) {
  ...
  if (authData && authData.access_token) {
    ...
    console.log("✅ Loaded token from local auth storage.");
  }
}

if (!TOKEN) {
  console.error("❌ No Antigravity Token found in config or env.");
  process.exit(1);
}
