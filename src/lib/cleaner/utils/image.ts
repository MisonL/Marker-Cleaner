 ...
 */
export function estimateTextureComplexity(pixels: Uint8Array, width: number, height: number): number {
  const target = 320;
  const scale = width > target ? target / width : 1;
  const dw = Math.max(8, Math.round(width * scale)), dh = Math.max(8, Math.round(height * scale));
  ...
  for (let y = 0; y < dh; y++) {
    ...
    for (let x = 0; x < dw; x++) {
      ...
      gray[y * dw + x] = 0.299 * (pixels[idx]??0) + 0.587 * (pixels[idx+1]??0) + 0.114 * (pixels[idx+2]??0);
    }
  }
  ...
  for (let y = 0; y < dh; y++) {
    for (let x = 0; x < dw; x++) {
      const gx = -getG(x-1,y-1) - 2*getG(x-1,y) - getG(x-1,y+1) + getG(x+1,y-1) + 2*getG(x+1,y) + getG(x+1,y+1);
      const gy = -getG(x-1,y-1) - 2*getG(x,y-1) - getG(x+1,y-1) + getG(x-1,y+1) + 2*getG(x,y+1) + getG(x+1,y+1);
      acc += Math.abs(gx) + Math.abs(gy); cnt++;
    }
  }
...
export function mergeBoxes(base: BoundingBox[], extra: BoundingBox[]): BoundingBox[] {
  ...
  const iou = (a: BoundingBox, b: BoundingBox) => {
    ...
    const ua = (a.xmax-a.xmin)*(a.ymax-a.ymin), ub = (b.xmax-b.xmin)*(b.ymax-b.ymin), union = ua + ub - inter;
  ...
  for (const b of extra) {
    ...
    for (let i = 0; i < out.length; i++) {
        ...
        const areaA = Math.max(0, (a.xmax-a.xmin)*(a.ymax-a.ymin)), areaB = Math.max(0, (b.xmax-b.xmin)*(b.ymax-b.ymin));
        const ratio = (areaA > 0 && areaB > 0) ? Math.min(areaA, areaB)/Math.max(areaA, areaB) : 0;
        if (overlap > 0.75 || (overlap > 0.55 && ratio > 0.55)) {
            out[i] = { ymin: Math.min(a.ymin, b.ymin), xmin: Math.min(a.xmin, b.xmin), ymax: Math.max(a.ymax, b.ymax), xmax: Math.max(a.xmax, b.xmax) };
            merged = true; break;
        }
    }
 ...
 * 转换归一化坐标到像素坐标并外扩
 */
export function toPixelRect(b: BoundingBox, width: number, height: number, paddingPx: number) {
  const clamp01 = (n: number) => Math.min(1, Math.max(0, n));
  const xmin = clamp01(Math.min(b.xmin, b.xmax)), xmax = clamp01(Math.max(b.xmin, b.xmax));
  const ymin = clamp01(Math.min(b.ymin, b.ymax)), ymax = clamp01(Math.max(b.ymin, b.ymax));
  return {
