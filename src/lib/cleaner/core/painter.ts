import { isMarkerColor, isLikelyMarkPixel, getDirectionalAverage, isStrongMarkColorForInpaint, isOverlayLikeStrong } from "../utils/color";
 ...
 * 获取邻域均值 (用于无法插值时的兜底)
 */
export function getNeighborAverage(pixels: Uint8Array, width: number, height: number, x: number, y: number): [number, number, number] {
  let sr = 0, sg = 0, sb = 0, sc = 0;
  for (let dy = -1; dy <= 1; dy++) {
    const ny = y + dy; if (ny < 0 || ny >= height) continue;
    for (let dx = -1; dx <= 1; dx++) {
      const nx = x + dx; if (nx < 0 || nx >= width || (nx === x && ny === y)) continue;
      const idx = (ny * width + nx) * 4;
      sr += pixels[idx] ?? 0; sg += pixels[idx + 1] ?? 0; sb += pixels[idx + 2] ?? 0; sc++;
    }
  }
  if (sc === 0) return [pixels[(y * width + x) * 4] ?? 0, pixels[(y * width + x) * 4 + 1] ?? 0, pixels[(y * width + x) * 4 + 2] ?? 0];
 ...
 * 评分候选框边缘的标记色浓度
 */
export function scoreFrameMarkerPixels(ctx: CleanerContext, rect: { x1: number; y1: number; x2: number; y2: number }, band: number, step = 3): number {
  ...
  for (let y = y1; y < Math.min(y1 + b, y2); y += step) { for (let x = x1; x < x2; x += step) countIfMarker(x, y); }
  for (let y = Math.max(y2 - b, y1); y < y2; y += step) { for (let x = x1; x < x2; x += step) countIfMarker(x, y); }
...
export function paintRectFrame(
  ...
  const midTop = y1 + Math.floor(b / 2), midBottom = y2 - 1 - Math.floor(b / 2);
  const midLeft = x1 + Math.floor(b / 2), midRight = x2 - 1 - Math.floor(b / 2);
  const edgeSearchY = Math.max(40, Math.min(280, Math.round(h * 0.45))), edgeSearchX = Math.max(40, Math.min(420, Math.round(w * 0.45)));

  const findBestRowNear = (s: number, e: number) => {
    let bestY = -1, bestRun = 0, step = 2;
    ...
    for (let yy = Math.max(y1, start); yy <= Math.min(y2-1, end); yy += step) {
      ...
      if (run > bestRun) { bestRun = run; bestY = yy; }
    }
    return { y: bestY, run: bestRun };
  };

  const findBestColNear = (s: number, e: number) => {
    let bestX = -1, bestRun = 0, step = 2;
    const start = Math.min(s, e), end = Math.max(s, e);
    for (let xx = Math.max(x1, start); xx <= Math.min(x2-1, end); xx += step) {
      const run = maxRunInCol(xx, step);
      if (run > bestRun) { bestRun = run; bestX = xx; }
    }
    return { x: bestX, run: bestRun };
  };

  const topScan = findBestRowNear(y1, y1 + edgeSearchY), bottomScan = findBestRowNear(y2 - edgeSearchY, y2 - 1);
  const leftScan = findBestColNear(x1, x1 + edgeSearchX), rightScan = findBestColNear(x2 - edgeSearchX, x2 - 1);
  ...
  const getPerpSamples = (x: number, y: number, dir: "horizontal" | "vertical", baseOffset: number) => {
    for (let extra = 0; extra <= 12; extra += 2) {
      const offset = baseOffset + extra, samples: Array<[number, number, number]> = [];
      if (dir === "horizontal") {
        for (const yy of [y - offset, y + offset]) if (yy >= 0 && yy < height) {
          const idx = (yy * info.width + x) * 4; samples.push([pixels[idx]??0, pixels[idx+1]??0, pixels[idx+2]??0]);
        }
      } else {
        for (const xx of [x - offset, x + offset]) if (xx >= 0 && xx < width) {
          const idx = (y * info.width + xx) * 4; samples.push([pixels[idx]??0, pixels[idx+1]??0, pixels[idx+2]??0]);
        }
      }
      if (samples.length >= 1) {
        ...
        const [s0, s1] = samples;
        const sideDiff = s0 && s1 ? Math.abs(s0[0]-s1[0])+Math.abs(s0[1]-s1[1])+Math.abs(s0[2]-s1[2]) : 99999;
  ...
  const forcePaint = !!options?.force || ((hitTop?1:0)+(hitBottom?1:0)+(hitLeft?1:0)+(hitRight?1:0)) >= (conservative ? 3 : 2);

  const computeReplacement = (x: number, y: number, mode: "horizontal" | "vertical" | "corner") => {
    const idx = (y * info.width + x) * 4, r = pixels[idx]??0, g = pixels[idx+1]??0, bb = pixels[idx+2]??0;
    const baseOffset = b + 3;
    const perp = mode === "horizontal" ? getPerpSamples(x, y, "horizontal", baseOffset) : mode === "vertical" ? getPerpSamples(x,y,"vertical",baseOffset) : { avg: getNeighborAverage(pixels, width, height, x, y), sideDiff: 9999, sampleCount: 0 };
    const relaxed = mode === "horizontal" ? getDirectionalAverage(pixels, width, height, x, y, true) : mode === "vertical" ? getDirectionalAverage(pixels, width, height, x, y, false) : getNeighborAverage(pixels, width, height, x, y);
  ...
  const paintBand = (cy: number, isHoriz: boolean) => {
    ...
    for (let c = from; c <= to; c++) {
      ...
      const flush = () => {
        if (run.length >= minRun) {
          for (const it of run) {
            ...
            pixels[idx] = it.nr; pixels[idx + 1] = it.ng; pixels[idx + 2] = it.nb;
      ...
      for (let i = 0; i < len; i++) {
        const x = isHoriz ? x1 + i : c, y = isHoriz ? c : y1 + i;
        const isCorner = isHoriz ? (x < x1 + half || x >= x2 - half) : (y < y1 + half || y >= y2 - half);
        const mode = isCorner ? "corner" : (isHoriz ? "horizontal" : "vertical");
        const res = computeReplacement(x, y, mode);
        if (res.shouldReplace) run.push({ pos: isHoriz ? x : y, nr: res.nr, ng: res.ng, nb: res.nb });
        else flush();
      }
      flush();
    }
  ...
  paintBand(lineTopY, true); paintBand(lineBottomY, true); paintBand(lineLeftX, false); paintBand(lineRightX, false);
}

/**
 * AI 框内的强色区域修复
 */
export function inpaintStrongColorInsideBoxes(ctx: CleanerContext, candidateBoxes: BoundingBox[]): number {
  ...
  for (const b of candidateBoxes) {
    ...
    for (let y = rect.y1; y < rect.y2; y++) {
      for (let x = rect.x1; x < rect.x2; x++) {
        ...
        if (isStrongMarkColorForInpaint(ctx.pixels[idx]??0, ctx.pixels[idx+1]??0, ctx.pixels[idx+2]??0)) mask[y * width + x] = 1;
      }
    }
  }
 ...
 */
export function inpaintStrongColorColumnsInsideBoxes(ctx: CleanerContext, candidateBoxes: BoundingBox[]): number {
  if (candidateBoxes.length === 0) return 0;
  ...
  for (const b of candidateBoxes) {
    ...
    const rectW = rect.x2 - rect.x1, rectH = rect.y2-rect.y1, edgeBand = Math.max(6, Math.min(18, Math.round(Math.min(rectW, rectH) * 0.06)));
    const runThreshold = Math.max(28, Math.round(rectH * 0.28)), countThreshold = Math.max(36, Math.round(rectH * 0.22));

    const testColumn = (x: number) => {
      let count = 0, run = 0, bestRun = 0;
      for (let y = rect.y1; y < rect.y2; y++) {
        const idx = (y * info.width + x) * 4;
        if (isOverlayLikeStrong(pixels[idx]??0, pixels[idx+1]??0, pixels[idx+2]??0)) { count++; run++; if (run > bestRun) bestRun = run; } else run = 0;
      }
      if (bestRun >= runThreshold && count >= countThreshold) {
        for (let y = rect.y1; y < rect.y2; y++) {
           ...
           if (isOverlayLikeStrong(pixels[idx]??0, pixels[idx+1]??0, pixels[idx+2]??0)) {
             for (let dy = -1; dy <= 1; dy++) {
               const ny = y + dy; if (ny >= rect.y1 && ny < rect.y2) {
                 for (let dx = -1; dx <= 1; dx++) {
                   const nx = x + dx; if (nx >= rect.x1 && nx < rect.x2) mask[ny * width + nx] = 1;
                 }
               }
             }
           }
